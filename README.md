...
## 🛸 구현해야 하는 기능(목록)
- 1부터 9까지 서로 다른 수로 이루어진 3자리의 수를 랜덤인 문자열로 만들기
- 사용자로부터 숫자를 문자열로 입력 받기
- 사용자로부터 입력 받은 문자열 유효성 검사
  - 각각의 문자가 정수인지 확인
  - 문자열의 길이가 3인지 확인
  - 중복여부 확인
- 사용자가 입력한 문자열과 랜덤인 문자열의 각 숫자를 비교해서 스트라이크 및 볼 카운트 판정
- 3개의 스트라이크가 나오면 게임이 종료
- 게임을 종료한 후 사용자에게 게임을 재시작(1), 완전 종료(2) 여부 확인